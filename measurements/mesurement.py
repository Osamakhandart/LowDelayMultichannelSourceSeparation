...
# Set the output device explicitly (replace with the correct device index)
# output_device_index = 1  # Change this to the index of your desired output device
# sd.default.device = output_device_index

# 🎧 Load two different sound sources (e.g., speech or music files)
source1, sr1 = sf.read("/home/ZA/Music/Media project/BSS_MP/BSS_MP/measurements/Audio Dataset/speech-female.wav")  # First speaker's audio
source2, sr2 = sf.read("/home/ZA/Music/Media project/BSS_MP/BSS_MP/measurements/Audio Dataset/singing.wav")  # Second speaker's audio
...
source1 = source1[:minlen] / np.max(np.abs(source1))
source2 = source2[:minlen] / np.max(np.abs(source2))
